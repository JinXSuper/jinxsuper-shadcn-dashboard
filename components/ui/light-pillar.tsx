...
const LightPillar: React.FC<LightPillarProps> = ({
  topColor = '#FF8C00', // Adjusted to match the dashboard's orange theme
  bottomColor = '#FFFFFF',
  intensity = 0.6,
  rotationSpeed = 0.2,
  interactive = false,
  ...
  useEffect(() => {
    ...
    const parse = (hex: string) => { const c = new THREE.Color(hex); return new THREE.Vector3(c.r, c.g, c.b); };
    materialRef.current.uniforms.uTopColor.value = parse(topColor);
  }, [topColor]);

  useEffect(() => {
    if (!materialRef.current) return;
    const parse = (hex: string) => { const c = new THREE.Color(hex); return new THREE.Vector3(c.r, c.g, c.b); };
    materialRef.current.uniforms.uBottomColor.value = parse(bottomColor);
  }, [bottomColor]);
  ...
  return <div ref={containerRef} className={`fixed inset-0 -z-50 size-full ${className}`} style={{ mixBlendMode }} />;
};
